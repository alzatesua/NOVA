...
export default function FacturaForm({ bodegaId, sucursalId, onFacturaCreada }) {
  ...
  return (
    <>
              ...
              <React.Fragment key={paso.numero}>
                ...
                <div className="flex flex-col items-center flex-1 min-w-0">
                  <button
                    ...
                    className={`
                      w-7 h-7 sm:w-8 sm:h-8 lg:w-10 lg:h-10 rounded-full flex items-center justify-center font-bold text-xs sm:text-sm
                  ...
                  <span className={`
                    text-[9px] sm:text-[10px] font-semibold text-center truncate
                    ${esActivo
                ...
                {index < pasos.length - 1 && (
                  <div className={`
                    h-0.5 flex-1 mx-0.5 sm:mx-1 rounded-full transition-all duration-300
      ...
      <div className="bg-white dark:!bg-slate-900 rounded-lg shadow-sm border border-slate-200 dark:!border-slate-800 min-h-[300px] sm:min-h-[350px]">
        {/* Paso 1: Cliente */}
        {pasoActual === 1 && (
          <div className="p-2.5 sm:p-3">
            <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3">
              <div className="p-1 sm:p-1.5 bg-blue-100 dark:bg-blue-900/30 rounded-lg">
                <UserCircleIcon className="h-4 w-4 sm:h-5 sm:w-5 text-blue-600 dark:text-blue-400" />
              </div>
              <div>
                <h3 className="text-sm sm:text-base lg:text-lg font-bold text-slate-900 dark:!text-slate-100">Información del Cliente</h3>
                <p className="text-[10px] sm:text-xs text-slate-500 dark:!text-slate-400">Busca o crea un cliente (opcional)</p>
              </div>
            </div>
            <ClienteSelector cliente={cliente} onClienteChange={setCliente} />

            {/* Selector de Tipo de Factura */}
            <div className="mt-3 sm:mt-4 bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900 rounded-lg p-2.5 sm:p-3 border border-slate-200 dark:border-slate-700">
              <div className="flex items-center gap-1.5 sm:gap-2 mb-2">
                <DocumentCheckIcon className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-slate-600 dark:text-slate-400" />
                <span className="text-xs sm:text-sm font-semibold text-slate-700 dark:text-slate-300">Tipo de Factura</span>
              </div>
              <div className="flex flex-col sm:flex-row gap-1.5 sm:gap-2">
                ...
                >
                  ...
                  <span className="text-[9px] sm:text-xs opacity-80 hidden sm:inline">(Paga al momento)</span>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="text-[9px] sm:text-xs opacity-80 hidden sm:inline">(Paga después)</span>
                </button>
              </div>
              {tipoFactura === 'credito' && (
                <div className="mt-1.5 sm:mt-2 p-1.5 sm:p-2 bg-blue-50 dark:bg-blue-900/20 rounded-md border border-blue-200 dark:border-blue-800">
        ...
        {pasoActual === 2 && (
          <div className="p-2 sm:p-3 lg:p-4">
            <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3">
              <div className="p-1 sm:p-1.5 bg-sky-100 dark:bg-sky-900/30 rounded-lg">
                <ShoppingCartIcon className="h-4 w-4 sm:h-5 sm:w-5 text-sky-600 dark:text-sky-400" />
              </div>
              <div>
                <h3 className="text-sm sm:text-base lg:text-lg font-bold text-slate-900 dark:!text-slate-100">Productos</h3>
                <p className="text-[10px] sm:text-xs text-slate-500 dark:!text-slate-400">Agrega productos a la venta</p>
              </div>
            </div>

            {!bodegaId ? (
              <div className="text-center py-4 sm:py-6 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border-2 border-dashed border-yellow-200 dark:border-yellow-800">
                <p className="text-[10px] sm:text-sm text-yellow-700 dark:text-yellow-400 font-medium px-2">Seleccione una bodega para agregar productos</p>
              </div>
            ) : productos.length === 0 ? (
              <div className="text-center py-6 sm:py-8 bg-slate-50 dark:!bg-slate-800 rounded-lg border border-slate-200 dark:!border-slate-700">
                <ShoppingCartIcon className="h-8 w-8 sm:h-10 sm:w-10 mx-auto text-slate-400 dark:text-slate-600 mb-2" />
                <p className="text-xs sm:text-sm text-slate-600 dark:!text-slate-400 mb-2 sm:mb-3 px-2">No hay productos agregados</p>
                <button
                  onClick={agregarFilaProducto}
                  className="inline-flex items-center gap-1 sm:gap-1.5 px-3 sm:px-4 py-1.5 sm:py-2 bg-sky-400 text-white text-[10px] sm:text-sm font-semibold rounded-lg hover:bg-sky-500 transition-colors"
                >
                  <PlusIcon className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                  <span className="hidden xs:inline">Agregar primer producto</span>
              ...
              <div className="space-y-1.5 sm:space-y-2">
                ...
                {productos.map((fila, index) => (
                  <div key={fila.fila_id} className="bg-slate-50 dark:!bg-slate-800 rounded-lg border border-slate-200 dark:!border-slate-700 p-2 sm:p-3">
                    <div className="flex items-start gap-1.5 sm:gap-2">
                      {/* Número de fila */}
                      <div className="flex-shrink-0 w-5 h-5 sm:w-6 sm:h-6 lg:w-7 lg:h-7 bg-blue-600 text-white rounded-full flex items-center justify-center text-[10px] sm:text-xs lg:text-sm font-bold">
                        {index + 1}
                      </div>
                        ...
                        {fila.producto_seleccionado ? (
                          ...
                          <div className="space-y-1.5 sm:space-y-2">
                            <div className="flex items-center justify-between gap-1.5 sm:gap-2">
                              <div className="flex-1 min-w-0">
                                <p className="text-xs sm:text-sm lg:text-base font-bold text-slate-900 dark:!text-slate-100 truncate">
                                  {fila.nombre}
                                </p>
                                <p className="text-[9px] sm:text-xs text-slate-500 dark:!text-slate-400 truncate">
                                  SKU: {fila.sku} • Disp: {fila.disponible}
                                </p>
                              </div>
                              <button
                                onClick={() => toggleBuscador(fila.fila_id)}
                                className="px-1.5 sm:px-2 py-1 text-[9px] sm:text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors flex-shrink-0"
                              >
                            ...
                            {/* Cantidad y precio */}
                            <div className="grid grid-cols-3 gap-1 sm:gap-2 text-[10px] sm:text-xs lg:text-sm">
                              <div>
                                <label className="block text-[9px] sm:text-[10px] lg:text-xs text-slate-600 dark:!text-slate-400 mb-0.5 sm:mb-1">Cantidad</label>
                                <input
                                  type="number"
                                  min="1"
                                  max={fila.disponible}
                              ...
                              <div>
                                <label className="block text-[9px] sm:text-[10px] lg:text-xs text-slate-600 dark:!text-slate-400 mb-0.5 sm:mb-1">Precio</label>
                                <p className="px-1.5 sm:px-2 py-1 bg-slate-100 dark:bg-slate-700 rounded text-center font-bold text-slate-900 dark:!text-slate-100 text-[10px] sm:text-xs">
                                  ${parseFloat(fila.precio).toFixed(2)}
                                </p>
                              </div>
                              <div>
                                <label className="block text-[9px] sm:text-[10px] lg:text-xs text-slate-600 dark:!text-slate-400 mb-0.5 sm:mb-1">Subtotal</label>
                                <p className="px-1.5 sm:px-2 py-1 bg-emerald-100 dark:bg-emerald-900/30 rounded text-center font-bold text-emerald-700 dark:text-emerald-400 text-[10px] sm:text-xs">
                          ...
                          <div className="space-y-1.5 sm:space-y-2">
                            <div className="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
                              <MagnifyingGlassIcon className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-slate-500 dark:!text-slate-400" />
                      ...
                      <button
                        ...
                        className="flex-shrink-0 p-1 sm:p-1.5 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"
                        title="Eliminar fila"
                      >
                ...
                {/* Resumen total */}
                <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-lg p-2 sm:p-3 text-white">
                  <div className="flex justify-between items-center">
                    <span className="text-xs sm:text-sm lg:text-base font-bold">
                      Total ({productos.filter(p => p.producto_seleccionado).length} {productos.filter(p => p.producto_seleccionado).length === 1 ? 'prod' : 'prods'}):
                    </span>
                ...
                <button
                  onClick={agregarFilaProducto}
                  disabled={!bodegaId}
                  className="flex items-center justify-center gap-1 sm:gap-1.5 w-full px-2 sm:px-3 py-1.5 sm:py-2 text-[10px] sm:text-xs bg-sky-50 dark:bg-sky-900/20 text-sky-700 dark:text-sky-400 rounded-lg hover:bg-sky-100 dark:hover:bg-sky-900/30 transition-colors font-semibold border border-sky-200 dark:border-sky-800"
        ...
        {pasoActual === 3 && (
          <div className="p-2 sm:p-3 lg:p-4">
            <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3">
              <div className="p-1 sm:p-1.5 bg-sky-100 dark:bg-sky-900/30 rounded-lg">
                <CreditCardIcon className="h-4 w-4 sm:h-5 sm:w-5 text-sky-600 dark:text-sky-400" />
              </div>
              <div>
                <h3 className="text-sm sm:text-base lg:text-lg font-bold text-slate-900 dark:!text-slate-100">
                  {tipoFactura === 'credito' ? 'Crédito' : 'Métodos de Pago'}
                </h3>
                <p className="text-[10px] sm:text-xs text-slate-500 dark:!text-slate-400">
                  {tipoFactura === 'credito' ? 'Configura el día de pago y cuotas' : 'Configura cómo se realizará el pago'}
                </p>
              </div>
            </div>
            ...
            {tipoFactura === 'credito' ? (
                ...
                <div className="space-y-2 sm:space-y-3 mb-2 sm:mb-3">
                    ...
                    >
                      ...
                      {[...Array(31)].map((_, i) => (
                        <option key={i + 1} value={i + 1}>
                          Día {i + 1}{i + 1 === 15 && ' (Quincena)'}{i + 1 === 30 && ' (Fin)'}
                        </option>
                      ))}
                    </select>
                  ...
                  <div className="bg-purple-50 dark:bg-purple-900/30 rounded-lg p-2 sm:p-3 border border-purple-200 dark:border-purple-700">
                    ...
                    >
                      ...
                      {[...Array(24)].map((_, i) => (
                        <option key={i + 1} value={i + 1}>
                          {i + 1} {i + 1 === 1 ? 'cuota' : 'cuotas'} {i + 1 === 1 && '(Único)'}{i + 1 === 2 && '(Bimes)'}{i + 1 === 3 && '(Trimes)'}{i + 1 === 6 && '(Semest)'}{i + 1 === 12 && '(Anual)'}
                        </option>
                      ))}
                    </select>
              ...
              <>
                ...
                {formasPago.length === 0 && (
                  <div className="mb-2 sm:mb-3 p-1.5 sm:p-2 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                    <p className="text-yellow-700 dark:text-yellow-400 text-[9px] sm:text-xs font-medium">
                ...
                <div className="space-y-1.5 sm:space-y-2 mb-2 sm:mb-3">
                      ...
                      <div className="flex gap-1 sm:gap-2">
                        ...
                        {pagos.length > 1 && (
                          <button
                            ...
                            className="p-1 sm:p-1.5 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors flex-shrink-0"
                            title="Eliminar"
                          >
                            <TrashIcon className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                          </button>
                  ...
                  <button
                    type="button"
                    onClick={agregarPago}
                    className="flex items-center justify-center gap-1 sm:gap-1.5 w-full px-2 py-1.5 text-[10px] sm:text-xs bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors font-semibold"
                  >
            ...
            <div className="bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-700 p-2 sm:p-3 rounded-lg border border-slate-200 dark:border-slate-600">
              <h4 className="text-[10px] sm:text-sm font-bold text-slate-900 dark:text-slate-100 mb-1.5 sm:mb-2">Resumen</h4>
              <div className="space-y-1 sm:space-y-1.5">
                <div className="flex justify-between items-center py-0.5 sm:py-1 border-b border-slate-200 dark:border-slate-600">
                  <span className="text-[10px] sm:text-xs text-slate-600 dark:text-slate-400">Subtotal:</span>
                  <span className="text-[10px] sm:text-xs font-bold text-slate-900 dark:text-slate-100">${calcularSubtotal().toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center py-0.5 sm:py-1 border-b border-slate-200 dark:border-slate-600">
                  <span className="text-[10px] sm:text-xs text-slate-600 dark:text-slate-400">IVA:</span>
                  <span className="text-[10px] sm:text-xs font-bold text-slate-900 dark:text-slate-100">${calcularIVA().toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center py-1 sm:py-1.5 bg-gradient-to-r from-blue-600 to-blue-700 rounded-lg px-1.5 sm:px-2">
                  <span className="text-xs sm:text-sm font-bold text-white">TOTAL:</span>
                  <span className="text-sm sm:text-base lg:text-xl font-bold text-white">${calcularTotal().toFixed(2)}</span>
                </div>
                {tipoFactura === 'contado' && (
                  <>
                    <div className="flex justify-between items-center py-0.5 sm:py-1">
                      <span className="text-[10px] sm:text-xs text-sky-700 dark:text-sky-400 font-semibold">Pagado:</span>
                      <span className="text-[10px] sm:text-sm font-bold text-sky-600 dark:text-sky-400">${calcularTotalPagado().toFixed(2)}</span>
                    </div>
                    {calcularCambio() > 0 && (
                      <div className="flex justify-between items-center py-0.5 sm:py-1 bg-blue-50 dark:bg-blue-900/30 rounded-lg px-1.5 sm:px-2">
                        <span className="text-[10px] sm:text-xs text-blue-700 dark:text-blue-400 font-semibold">Cambio:</span>
                        <span className="text-xs sm:text-base font-bold text-blue-600 dark:text-blue-400">${calcularCambio().toFixed(2)}</span>
                      </div>
                    )}
                  </>
                )}
                {tipoFactura === 'credito' && cuotas && (
                  <div className="flex justify-between items-center py-0.5 sm:py-1 bg-purple-50 dark:bg-purple-900/30 rounded-lg px-1.5 sm:px-2">
                    <span className="text-[10px] sm:text-xs text-purple-700 dark:text-purple-400 font-semibold">Valor/cuota:</span>
                    <span className="text-xs sm:text-base font-bold text-purple-600 dark:text-purple-400">${(calcularTotal() / parseInt(cuotas)).toFixed(2)}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="p-2 sm:p-3 lg:p-4">
            <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3">
              <div className="p-1 sm:p-1.5 bg-purple-100 dark:bg-purple-900/30 rounded-lg">
                <DocumentCheckIcon className="h-4 w-4 sm:h-5 sm:w-5 text-purple-600 dark:text-purple-400" />
              </div>
              <div>
                <h3 className="text-sm sm:text-base lg:text-lg font-bold text-slate-900 dark:!text-slate-100">Confirmar Venta</h3>
                <p className="text-[10px] sm:text-xs text-slate-500 dark:!text-slate-400">Revisa antes de finalizar</p>
              </div>
            </div>
            ...
            <div className="space-y-2 sm:space-y-3 max-h-[350px] sm:max-h-[450px] overflow-y-auto">
              {/* Cliente */}
              <div className="bg-slate-50 dark:!bg-slate-800 p-2 sm:p-2.5 rounded-lg border border-slate-200 dark:!border-slate-700">
                <h4 className="text-[10px] sm:text-xs font-semibold text-slate-700 dark:!text-slate-300 mb-1 sm:mb-1.5 flex items-center gap-1">
                  <UserCircleIcon className="h-3 w-3 sm:h-3.5 sm:w-3.5" />
                  Cliente
                </h4>
                {cliente ? (
                  <div>
                    <p className="text-[10px] sm:text-xs font-bold text-slate-900 dark:!text-slate-100 truncate">{cliente.nombre_completo}</p>
                    <p className="text-[9px] sm:text-[10px] text-slate-600 dark:!text-slate-400">{cliente.tipo_documento}: {cliente.numero_documento}</p>
                  </div>
                ) : (
                  <p className="text-[10px] sm:text-xs text-slate-500 dark:!text-slate-400">Consumidor Final</p>
                )}
              </div>
                  ...
                  {productos.filter(p => p.producto_seleccionado).map((p) => (
                    <div key={p.fila_id} className="flex justify-between items-center py-1 border-b border-slate-200 dark:!border-slate-700 last:border-0">
                      <div className="flex-1 min-w-0 mr-1.5 sm:mr-2">
                        <p className="text-[10px] sm:text-xs font-medium text-slate-900 dark:!text-slate-100 truncate">{p.nombre}</p>
                        <p className="text-[9px] sm:text-[10px] text-slate-500 dark:!text-slate-400">{p.cantidad} × ${parseFloat(p.precio).toFixed(2)}</p>
                      </div>
                      <p className="text-[10px] sm:text-xs font-bold text-slate-900 dark:!text-slate-100 flex-shrink-0">${(parseFloat(p.precio) * p.cantidad).toFixed(2)}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-slate-50 dark:!bg-slate-800 p-2 sm:p-2.5 rounded-lg border border-slate-200 dark:!border-slate-700">
                ...
                {tipoFactura === 'credito' ? (
                  <div className="space-y-1 sm:space-y-1.5">
                    ...
                    <div className="flex justify-between items-center text-[10px] sm:text-xs bg-purple-50 dark:bg-purple-900/20 rounded px-1.5 sm:px-2 py-1">
                    ...
                    {pagos.map((pago, index) => {
                      ...
                      return (
                        <div key={index} className="flex justify-between items-center text-[10px] sm:text-xs">
                          <span className="text-slate-700 dark:!text-slate-300 truncate flex-1 mr-2">{formaPago?.nombre || 'N/A'}</span>
                          <span className="font-semibold text-slate-900 dark:!text-slate-100 flex-shrink-0">${parseFloat(pago.monto).toFixed(2)}</span>
                        </div>
                      );
                    })}
                  </div>
              ...
              {/* Total final */}
              <div className="bg-gradient-to-r from-blue-600 to-blue-700 p-2 sm:p-3 rounded-lg text-white">
                <div className="flex justify-between items-center">
                  <span className="text-xs sm:text-sm lg:text-base font-bold">TOTAL:</span>
                  <span className="text-sm sm:text-base lg:text-2xl font-bold">${calcularTotal().toFixed(2)}</span>
                </div>
              </div>

              {/* Observaciones */}
              <div>
                <label className="block text-[10px] sm:text-xs font-semibold text-slate-700 dark:!text-slate-300 mb-1 sm:mb-1.5">
      ...
      <div className="flex justify-between items-center gap-1.5 sm:gap-2">
        {pasoActual > 1 && (
          <button
            ...
            className="flex items-center gap-0.5 sm:gap-1 px-2 sm:px-3 py-1 sm:py-1.5 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:!text-slate-200 text-[10px] sm:text-xs font-semibold rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors"
          >
        ...
        {pasoActual < 4 ? (
          <button
            ...
            className="flex items-center gap-0.5 sm:gap-1 px-2 sm:px-3 py-1 sm:py-1.5 bg-gradient-to-r from-blue-600 to-blue-700 text-white text-[10px] sm:text-xs font-semibold rounded-lg hover:from-blue-700 hover:to-blue-800 disabled:from-slate-400 disabled:to-slate-500 disabled:cursor-not-allowed transition-all shadow-md"
          >
          ...
          <button
            ...
            disabled={loading}
            className="flex items-center gap-1 sm:gap-1.5 px-3 sm:px-4 py-1.5 sm:py-2 lg:px-6 lg:py-2.5 bg-gradient-to-r from-sky-400 to-cyan-400 text-white text-[10px] sm:text-xs font-bold rounded-lg hover:from-sky-500 hover:to-cyan-500 disabled:from-slate-400 disabled:to-slate-500 disabled:cursor-not-allowed transition-all shadow-lg shadow-sky-500/30"
          >
            {loading ? (
              <>
              ...
              <>
                <CheckIcon className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                <span className="hidden xs:inline">Completar Venta</span>
                <span className="xs:hidden">Completar</span>
              </>
            )}
          </button>
        )}
      ...
      {mostrarConfirmacionMora && clienteMora && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4 animate-fadeIn">
          <div className="bg-white dark:!bg-slate-900 rounded-xl sm:rounded-2xl shadow-2xl max-w-md w-full sm:max-w-lg animate-scaleIn max-h-[95vh] sm:max-h-[90vh] overflow-y-auto">
            {/* Header */}
            <div className="bg-gradient-to-r from-red-600 to-orange-600 px-3 py-2 sm:px-6 sm:py-4 rounded-t-xl sm:rounded-t-2xl">
              <div className="flex items-center gap-2 sm:gap-3">
                <ExclamationTriangleIcon className="h-5 w-5 sm:h-8 sm:w-8 text-white flex-shrink-0" />
                <div>
                  <h3 className="text-base sm:text-xl font-bold text-white">Cliente en Mora</h3>
                  <p className="text-red-100 text-[10px] sm:text-sm">Advertencia de crédito</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-3 sm:p-6">
              <div className="bg-red-50 dark:!bg-red-900/20 border-2 border-red-200 dark:!border-red-800 rounded-xl p-2 sm:p-4 mb-3 sm:mb-4">
                <div className="space-y-1.5 sm:space-y-2">
                  ...
                  <div className="text-[10px] sm:text-sm text-red-700 dark:!text-red-400 space-y-1">
                    <p>• <strong className="text-[10px] sm:text-sm">Días de mora:</strong> {clienteMora.dias_mora} días</p>
                    {clienteMora.fecha_ultimo_pago && (
                      <p>• <strong className="text-[10px] sm:text-sm">Último pago:</strong> {new Date(clienteMora.fecha_ultimo_pago).toLocaleDateString('es-CO')}</p>
                    )}
                    {clienteMora.limite_credito && clienteMora.limite_credito !== '0.00' && (
                      <p>• <strong className="text-[10px] sm:text-sm">Límite de crédito:</strong> ${clienteMora.limite_credito}</p>
                    )}
                  </div>
                  {clienteMora.observaciones_mora && (
                    <div className="mt-1.5 sm:mt-2 pt-1.5 sm:pt-2 border-t border-red-300 dark:!border-red-700">
                      <p className="text-[9px] sm:text-xs italic text-red-600 dark:!text-red-500">
              ...
              <p className="text-[10px] sm:text-sm text-slate-600 dark:!text-slate-400 mb-3 sm:mb-4">
              ...
              <div className="bg-blue-50 dark:!bg-blue-900/20 border border-blue-200 dark:!border-blue-800 rounded-lg p-2 sm:p-3">
                <p className="text-[9px] sm:text-xs text-blue-800 dark:!text-blue-300">
            ...
            {/* Footer */}
            <div className={`flex gap-2 sm:gap-3 px-3 py-2 sm:px-6 sm:py-4 border-t ${'border-slate-200 dark:!border-slate-700'}`}>
              <button
                type="button"
                onClick={handleCancelarFacturaPorMora}
                className="flex-1 px-2 py-1.5 sm:px-4 sm:py-2.5 bg-slate-200 dark:!bg-slate-700 text-slate-700 dark:!text-slate-200 font-semibold rounded-lg sm:rounded-xl hover:bg-slate-300 dark:hover:!bg-slate-600 transition-colors text-[10px] sm:text-xs"
              >
                Cancelar Venta
              </button>
              <button
                ...
                className="flex-1 px-2 py-1.5 sm:px-4 sm:py-2.5 bg-gradient-to-r from-blue-600 to-blue-700 text-white font-semibold rounded-lg sm:rounded-xl hover:from-blue-700 hover:to-blue-800 transition-all shadow-lg shadow-blue-500/30 text-[10px] sm:text-xs"
              >
                Continuar
              </button>
            </div>
