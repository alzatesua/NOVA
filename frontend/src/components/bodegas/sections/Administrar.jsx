...
export default function Administrar({ sucursalSel, handleTabChange }) {
  return (
    <div className="bg-white/80 dark:!bg-slate-900/80 backdrop-blur-sm rounded-xl p-2 sm:p-3 md:p-5 border border-white/20 dark:!border-slate-800 shadow-lg">
      <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 mb-2 sm:mb-3 md:mb-4">
        <div className="p-1.5 sm:p-2 md:p-3 bg-gradient-to-br from-blue-600 to-blue-500 rounded-lg" style={{
          ...
          <AdjustmentsHorizontalIcon className="w-4 h-4 sm:w-5 sm:w-6 md:w-6 text-white" />
        </div>
        <div className="flex-1 min-w-0">
          <h3 className="text-sm sm:text-base md:text-xl font-bold text-slate-800 dark:!text-slate-100 leading-tight">
        ...
        {[
          ...
          return (
            <button
              key={index}
              onClick={card.action}
              className="group touch-target text-left p-2 sm:p-3 md:p-4 bg-white/60 dark:!bg-slate-800/60 hover:bg-white/80 dark:hover:!bg-slate-700/80 rounded-xl border border-slate-200/60 dark:!border-slate-700/60 hover:border-slate-300/60 dark:hover:border-slate-600/60 transition-all duration-300 hover:scale-[1.02] sm:hover:scale-105 hover:shadow-lg"
              style={{
                minHeight: 'clamp(6rem, 14vw, 8rem)'
              }}
            >
              <div className={`w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-gradient-to-br ${card.color} rounded-lg flex items-center justify-center mb-1.5 sm:mb-2 md:mb-3 group-hover:scale-110 transition-transform duration-300`}>
                <Icon className="w-4 h-4 sm:w-5 sm:w-5 md:w-6 text-white" />
              </div>
              ...
              <div className="mt-1.5 sm:mt-2 md:mt-3 flex items-center text-blue-600 dark:text-blue-400 text-[10px] sm:text-xs font-medium group-hover:gap-2 transition-all duration-300">
                Acceder <ChevronRightIcon className="w-3 h-3 sm:w-3.5" />
              </div>
            </button>
          );
        })}
