  ...
  return (
    <div className="bg-white/80 dark:!bg-slate-900/80 backdrop-blur-sm rounded-xl p-5 border border-white/20 dark:!border-slate-800 shadow-lg">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="text-base font-bold text-slate-800 dark:!text-slate-100">Centro de Administración</h3>
          <p className="text-xs text-slate-600 dark:!text-slate-400">Gestiona todas las operaciones de {sucursalSel?.nombre}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {[
          ...
          return (
            <button key={index} onClick={card.action} className="group text-left p-4 bg-white/60 dark:!bg-slate-800/60 hover:bg-white/80 dark:hover:!bg-slate-700/80 rounded-xl border border-slate-200/60 dark:!border-slate-700/60 hover:border-slate-300/60 dark:hover:border-slate-600/60 transition-all duration-300 hover:scale-105 hover:shadow-lg">
              ...
              <h4 className="text-sm font-semibold text-slate-800 dark:!text-slate-100 mb-1">{card.title}</h4>
              <p className="text-xs text-slate-600 dark:!text-slate-400">{card.description}</p>
              <div className="mt-3 flex items-center text-blue-600 dark:text-blue-400 text-xs font-medium group-hover:gap-2 transition-all duration-300">
