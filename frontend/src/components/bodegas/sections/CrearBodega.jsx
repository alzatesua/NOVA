  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm rounded-xl p-5 border border-white/20 shadow-lg">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="text-base font-bold text-slate-800">Nueva Bodega</h3>
          <p className="text-xs text-slate-600">Configura los detalles de tu nueva bodega</p>
        </div>
      </div>

      <form onSubmit={onCrearBodega} className="space-y-4">
        <div className="group">
          <label className="block text-xs font-semibold text-slate-700 mb-1.5">Nombre de la bodega</label>
          <input
            required
            className="w-full px-3 py-2 bg-white/70 border border-slate-200/60 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-300 placeholder:text-slate-400 text-sm"
        ...
        <div className="group">
          <label className="block text-xs font-semibold text-slate-700 mb-1.5">Tipo de bodega</label>
          <select
            className="w-full px-3 py-2 bg-white/70 border border-slate-200/60 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-300 text-sm"
        ...
        <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg p-4 border border-indigo-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-600 mt-0.5">Esta bodega se usará por defecto en todas las operaciones</p>
            </div>
        ...
        <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-lg p-4 border border-emerald-100">
          <div className="flex items-center justify-between">
            <div>
              <h4 className="text-sm font-semibold text-slate-800">Estado de la bodega</h4>
              <p className="text-xs text-slate-600 mt-0.5">Controla si la bodega estará disponible</p>
            </div>
            ...
            <div className="flex items-center bg-white/80 rounded-lg p-0.5 shadow-sm">
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md transition-all duration-200 ${
                  bodegaForm?.estatus ? 'bg-emerald-500 text-white shadow-sm shadow-emerald-500/30' : 'text-slate-600 hover:text-slate-800'
                }`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md transition-all duration-200 ${
                  !bodegaForm?.estatus ? 'bg-rose-500 text-white shadow-sm shadow-rose-500/30' : 'text-slate-600 hover:text-slate-800'
                }`}
              >
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-slate-200/60">
          <button type="button" onClick={onClose} className="px-4 py-2 text-slate-700 font-medium rounded-lg border border-slate-200 hover:bg-slate-50 hover:border-slate-300 transition-all duration-200 text-sm">Cancelar</button>
